...
(function () {
    ...
    class miniVU {
        ...
        onLoad() {
            ...
            if (hash !== "" && this.isMiniVUHash(hash)) {
                let viewName = hash.substr(2);
                if (viewName === "") {
                    this.loadDefaultView();
                } else {
        ...
        errorViewNotfound(fileName, err) {
            console.error(this.ERROR_MSGS.VIEW_NOT_FOUND + " File name: " + fileName + ", Views directory: " + this.CONFIG.viewsDir, err);
            /** Display default or custom "Not Found" page depending on config. */
            if (this.CONFIG.customNotFound) {
                this.go(this.CONFIG.customNotFound);
            } else {
